...
@QuarkusTest
class RandomDummyInvalidJsonBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomDummyInvalidJsonBodyPlaybook randomDummyInvalidJsonBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomDummyInvalidJsonBodyPlaybook = new RandomDummyInvalidJsonBodyPlaybook(simpleExecutor);
    ...
    void shouldRunForEmptyPayload() {
        ...
        randomDummyInvalidJsonBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        randomDummyInvalidJsonBodyPlaybook.run(data);
        Mockito.verify(testCaseListener, Mockito.times(12)).reportResult(Mockito.any(), Mockito.eq(data), Mockito.eq(httpResponse), Mockito.eq(ResponseCodeFamilyPredefined.FOURXX), Mockito.anyBoolean(), Mockito.eq(true));
    }

    @Test
    void shouldHaveToString() {
        Assertions.assertThat(randomDummyInvalidJsonBodyPlaybook).hasToString(randomDummyInvalidJsonBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomDummyInvalidJsonBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomDummyInvalidJsonBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }
}
