...
@QuarkusTest
class RandomNegativeDecimalBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomNegativeDecimalBodyPlaybook randomNegativeDecimalBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomNegativeDecimalBodyPlaybook = new RandomNegativeDecimalBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        randomNegativeDecimalBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        randomNegativeDecimalBodyPlaybook.run(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(randomNegativeDecimalBodyPlaybook).hasToString(randomNegativeDecimalBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomNegativeDecimalBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomNegativeDecimalBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveNegativeDecimalPayload() {
        Assertions.assertThat(randomNegativeDecimalBodyPlaybook.getPayload(null)).isEqualTo("-9.876987698769877E11");
    }
}
