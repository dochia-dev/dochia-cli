...
@QuarkusTest
class RandomNegativeIntegerBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomNegativeIntegerBodyPlaybook randomNegativeIntegerBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomNegativeIntegerBodyPlaybook = new RandomNegativeIntegerBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        randomNegativeIntegerBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        randomNegativeIntegerBodyPlaybook.run(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(randomNegativeIntegerBodyPlaybook).hasToString(randomNegativeIntegerBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomNegativeIntegerBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomNegativeIntegerBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveNegativeIntegerPayload() {
        Assertions.assertThat(randomNegativeIntegerBodyPlaybook.getPayload(null)).isEqualTo("-987698769876");
    }
}
