...
@QuarkusTest
class RandomPositiveIntegerBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomPositiveIntegerBodyPlaybook randomPositiveIntegerBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomPositiveIntegerBodyPlaybook = new RandomPositiveIntegerBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        randomPositiveIntegerBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        randomPositiveIntegerBodyPlaybook.run(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(randomPositiveIntegerBodyPlaybook).hasToString(randomPositiveIntegerBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomPositiveIntegerBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomPositiveIntegerBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHavePositiveIntegerPayload() {
        Assertions.assertThat(randomPositiveIntegerBodyPlaybook.getPayload(null)).isEqualTo("987698769876");
    }
}
