...
@QuarkusTest
class RandomPositiveDecimalBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private RandomPositiveDecimalBodyPlaybook randomPositiveDecimalBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        randomPositiveDecimalBodyPlaybook = new RandomPositiveDecimalBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        randomPositiveDecimalBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        randomPositiveDecimalBodyPlaybook.run(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(randomPositiveDecimalBodyPlaybook).hasToString(randomPositiveDecimalBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(randomPositiveDecimalBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(randomPositiveDecimalBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHavePositiveDecimalPayload() {
        Assertions.assertThat(randomPositiveDecimalBodyPlaybook.getPayload(null)).isEqualTo("987698769876.9876987698769876987698769876987698769876");
    }
}
