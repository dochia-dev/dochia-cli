...
@QuarkusTest
class EmptyJsonBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private EmptyJsonBodyPlaybook emptyJsonBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        emptyJsonBodyPlaybook = new EmptyJsonBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        emptyJsonBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void shouldHaveToString() {
        Assertions.assertThat(emptyJsonBodyPlaybook).hasToString(emptyJsonBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(emptyJsonBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(emptyJsonBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveNullPayload() {
        Assertions.assertThat(emptyJsonBodyPlaybook.getPayload(null)).isEqualTo("{}");
    }
}
