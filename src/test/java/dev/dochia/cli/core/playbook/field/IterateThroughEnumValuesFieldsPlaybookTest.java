...
@QuarkusTest
class IterateThroughEnumValuesFieldsPlaybookTest
 {
    ServiceCaller serviceCaller;
    @InjectSpy
    TestCaseListener testCaseListener;
    FieldsIteratorExecutor executor;
    private IterateThroughEnumValuesFieldsPlaybook iterateThroughEnumValuesFieldsPlaybook;
    ...
    void setup() {
        ...
        iterateThroughEnumValuesFieldsPlaybook = new IterateThroughEnumValuesFieldsPlaybook(executor);
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(iterateThroughEnumValuesFieldsPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldHaveToString() {
        Assertions.assertThat(iterateThroughEnumValuesFieldsPlaybook).hasToString("IterateThroughEnumValuesFieldsPlaybook");
    }

    @Test
    void shouldSkipForNonBodyMethods() {
        Assertions.assertThat(iterateThroughEnumValuesFieldsPlaybook.skipForHttpMethods()).isEmpty();
    }
    ...
    void shouldSkipIfNotEnum() {
        ...
        Mockito.when(data.getPayload()).thenReturn("""
                    {"myField": 3}
                """);
        iterateThroughEnumValuesFieldsPlaybook.run(data);
