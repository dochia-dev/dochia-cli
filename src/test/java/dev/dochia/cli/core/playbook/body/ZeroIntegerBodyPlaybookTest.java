...
@QuarkusTest
class ZeroIntegerBodyPlaybookTest {
    private ServiceCaller serviceCaller;
    @InjectSpy
    private TestCaseListener testCaseListener;
    private ZeroIntegerBodyPlaybook zeroIntegerBodyPlaybook;

    @BeforeEach
    void setup() {
        serviceCaller = Mockito.mock(ServiceCaller.class);
        SimpleExecutor simpleExecutor = new SimpleExecutor(testCaseListener, serviceCaller);
        zeroIntegerBodyPlaybook = new ZeroIntegerBodyPlaybook(simpleExecutor);
        ReflectionTestUtils.setField(testCaseListener, "testReportsGenerator", Mockito.mock(TestReportsGenerator.class));
    }

    @Test
    void shouldNotRunForEmptyPayload() {
        zeroIntegerBodyPlaybook.run(Mockito.mock(PlaybookData.class));
    ...
    void givenAHttpMethodWithPayload_whenApplyingTheMalformedJsonPlaybook_thenTheResultsAreCorrectlyReported() {
        ...
        zeroIntegerBodyPlaybook.run(data);
    ...
    void shouldHaveToString() {
        Assertions.assertThat(zeroIntegerBodyPlaybook).hasToString(zeroIntegerBodyPlaybook.getClass().getSimpleName());
    }

    @Test
    void shouldHaveDescription() {
        Assertions.assertThat(zeroIntegerBodyPlaybook.description()).isNotBlank();
    }

    @Test
    void shouldSkipForNonHttpBodyMethods() {
        Assertions.assertThat(zeroIntegerBodyPlaybook.skipForHttpMethods()).contains(HttpMethod.GET, HttpMethod.DELETE);
    }

    @Test
    void shouldHaveZeroIntegerPayload() {
        Assertions.assertThat(zeroIntegerBodyPlaybook.getPayload(null)).isEqualTo("0");
    }
}
