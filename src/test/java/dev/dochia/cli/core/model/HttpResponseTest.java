...
class HttpResponseTest {
    ...
    @Test
    void givenAStringJson_whenCreatingADochiaResponseObject_thenTheStringIsParsedAsAJsonElement() {
        String body = "{'test': 'value'}";
        HttpResponse httpResponse = HttpResponse.from(200, body, "DELETE", 2);

        Assertions.assertThat(httpResponse.getJsonBody()).isNotNull();
    }
    ...
    void shouldTestResponseTimeExceeding(int responseTime, int maxResponseTime, boolean expected) {
        HttpResponse response = HttpResponse.builder().responseTimeInMs(responseTime).build();
    ...
    void shouldCheckValidErrorCode(int code, boolean expected) {
        HttpResponse response = HttpResponse.builder().responseCode(code).build();
    ...
    void shouldReturnProperExceptionalResponse(String message, int responseCode) {
        ...
        HttpResponse.ExceptionalResponse response = HttpResponse.getResponseByException(exception);
