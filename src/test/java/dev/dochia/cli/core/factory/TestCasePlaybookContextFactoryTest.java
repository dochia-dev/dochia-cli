...
class TestCasePlaybookContextFactoryTest {
    ...
    private PlaybookDataFactory playbookDataFactory;
    ...
    void setup() {
        ...
        playbookDataFactory = new PlaybookDataFactory(filesArguments, processingArguments, globalContext, validDataFormat, filterArguments);
    ...
    private static void assertPropertiesExistInRequestPropertyTypes(PlaybookData firstData) {
        ...
        Assertions.assertThat(allFieldsAsDochiaFields).hasSizeLessThanOrEqualTo(firstData.getRequestPropertyTypes().size());
        Assertions.assertThat(firstData.getRequestPropertyTypes().keySet()).containsAll(allFieldsAsDochiaFields.stream().map(DochiaField::getName).toList());
    }
    ...
    void givenAContract_whenParsingThePathItemDetailsForPost_thenCorrectFuzzingDataAreBeingReturned() throws Exception {
        ...
        Assertions.assertThat(firstData.getAllFieldsAsDochiaFields()).hasSizeLessThanOrEqualTo(data.getFirst().getRequestPropertyTypes().size());
    ...
    void shouldNotGenerateRequestBodyWhenPostButSchemaEmpty() throws Exception {
        ...
        Assertions.assertThat(data.getFirst().getAllFieldsAsDochiaFields()).hasSizeLessThanOrEqualTo(data.getFirst().getRequestPropertyTypes().size());
    }
    ...
    void shouldResolveExampleCrossSchemas() throws Exception {
        ...
        PlaybookData firstData = data.getFirst();
        Assertions.assertThat(firstData.getPayload()).contains("9999dochia", "prev owner");
        assertPropertiesExistInRequestPropertyTypes(firstData);
    }
    ...
    void shouldNotUseExamplesWhenRequestBodyFlagDisabled() throws Exception {
        ...
        PlaybookData secondData = data.get(1);
    ...
    void shouldReturnRequiredFieldsWhenAllOfSchemaAndRequiredInRoot() throws Exception {
        ...
        Assertions.assertThat(playbookData.getAllRequiredFields()).containsExactly("breed", "id").doesNotContain("color");
        assertPropertiesExistInRequestPropertyTypes(playbookData);
    }
    ...
    void shouldReturnRequiredFieldsWhenAllOfSchemaAndRequiredPropertiesInBothSchemas() throws Exception {
        ...
        Assertions.assertThat(data).hasSize(1);
        PlaybookData playbookData = data.getFirst();
        Assertions.assertThat(playbookData.getAllRequiredFields()).containsExactly("legs", "breed", "id").doesNotContain("color");
        assertPropertiesExistInRequestPropertyTypes(playbookData);
    }
    ...
    private List<PlaybookData> setupFuzzingData(String path, String contract, boolean resolve) throws IOException {
        ...
        globalContext.setOpenAPI(openAPI);

        Mockito.when(filesArguments.isNotUrlParam(Mockito.anyString())).thenReturn(true);
        return playbookDataFactory.fromPathItem(path, pathItem, openAPI);
    }
    ...
    void shouldGenerateCombinationWhenXxxArraysAndSimpleTypes() throws IOException {
        ...
        PlaybookData data = dataList.get(6);
        ...
        PlaybookData createData = dataList.get(7);
    ...
    void shouldGenerateCombinationsWhenXxxAsInlineSchemas() throws IOException {
        ...
        PlaybookData zeroData = dataList.getFirst();
    ...
    void shouldProperlyGenerateOneOfAnyOfPayloads() throws Exception {
        ...
        long husky = dataList.stream().map(PlaybookData::getPayload).filter(payload -> payload.contains("Husky")).count();
        long labrador = dataList.stream().map(PlaybookData::getPayload).filter(payload -> payload.contains("Labrador")).count();
    ...
    void shouldGenerateCrossPathReferenceParameters() throws Exception {
        ...
        Object parameter = globalContext.getObjectFromPathsReference("#/paths/~1v2~1account~1keys~1%7Bssh_key_identifier%7D/get/parameters/0");
    ...
    void shouldGenerateCrossPathReferenceHeaders() throws Exception {
        ...
        Object header = globalContext.getObjectFromPathsReference("#/paths/~1v2~11-clicks/get/responses/200/headers/ratelimit-limit");
    ...
    void shouldGenerateCrossPathParametersReferences() throws Exception {
        ...
        List<PlaybookData> getData = dataList.stream().filter(data -> data.getMethod() == HttpMethod.GET).toList();
        Assertions.assertThat(getData.stream().map(PlaybookData::getPayload)).hasSize(2).allMatch(data -> data.contains("ssh_key_identifier"));

        List<PlaybookData> deleteData = dataList.stream().filter(data -> data.getMethod() == HttpMethod.DELETE).toList();
        Assertions.assertThat(deleteData.stream().map(PlaybookData::getPayload)).hasSize(2).allMatch(data -> data.contains("ssh_key_identifier"));
    }
    ...
    void shouldRecordErrorWhenParamRefDoesNotExist() throws Exception {
        ...
        Assertions.assertThat(globalContext.getRecordedErrors()).isEmpty();

        List<Parameter> resolvedParameters = playbookDataFactory.getResolvedParameters(List.of(param1));
        Assertions.assertThat(resolvedParameters).isEmpty();
        Assertions.assertThat(globalContext.getRecordedErrors()).hasSize(1);
    }
    ...
    void shouldGenerateHeadersWhenParamsHaveContentType() throws Exception {
        ...
        List<DochiaHeader> metadata = getData.getHeaders().stream().filter(header -> header.getName().equals("x-metadata")).toList();
        ...
        List<DochiaHeader> badConstraints = getData.getHeaders().stream().filter(header -> header.getName().equals("Bad-Constraints")).toList();
        ...
        Assertions.assertThat(badConstraints.getFirst().getValue()).isEqualTo(OpenAPIModelGenerator.DEFAULT_STRING_WHEN_GENERATION_FAILS);
    }
    ...
    void shouldExtractMultiLevelExamples() {
        ...
        exampleMapObject.set("WORKER_COMPENSATION_PAYRATE_POST_PATCH", objectMapper.createObjectNode().put("value", "dochiaIsCool"));
        globalContext.getExampleMap().put("JSON_WORKER_EXAMPLES", new Example().value(exampleMapObject));
        ...
        Set<Object> examples = playbookDataFactory.extractExamples(mediaType);

        Assertions.assertThat(examples).hasSize(2).containsExactly("example2", "\"dochiaIsCool\"");
    }
    ...
    void shouldHaveContentWhenContentNotNull() {
        ...
        Assertions.assertThat(PlaybookDataFactory.hasContent(operation)).isTrue();
    }
