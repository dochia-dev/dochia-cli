...
@QuarkusTest
class ErrorsSimilarityDetectorTest {
    ...
    void testAreErrorsSimilar(String error1, String error2, boolean expectedSimilarity) {
        assertThat(ErrorsSimilarityDetector.areErrorsSimilar(error1, error2)).isEqualTo(expectedSimilarity);
    }
    ...
    void testAreErrorsSimilar_withBlankOrNullErrors(String error1) {
        ...
        assertThat(ErrorsSimilarityDetector.areErrorsSimilar(error1, error2)).isFalse();
    }

    @ParameterizedTest
    @MethodSource("provideNormalizeErrorMessageTestCases")
    void testNormalizeErrorMessage(String input, String expected) {
        assertThat(ErrorsSimilarityDetector.normalizeErrorMessage(input)).isEqualTo(expected);
    }
