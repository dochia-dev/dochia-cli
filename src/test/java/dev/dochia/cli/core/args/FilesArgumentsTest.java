...
class FilesArgumentsTest {
    ...
    @Test
    void shouldLoadPlaybooksConfig() throws IOException {
        FilesArguments filesArguments = new FilesArguments();
        ReflectionTestUtils.setField(filesArguments, "playbooksConfig", new File("src/test/resources/playbooksConfig.properties"));
        filesArguments.loadPlaybooksConfigProperties();
        String existingProperty = filesArguments.getPlaybookConfigProperties().getProperty("DummyAcceptHeaders.expectedResponseCode");
        String nonExistingProperty = filesArguments.getPlaybookConfigProperties().getProperty("DummyAcceptHeaders.expectedScenario");
    ...
    void shouldMergeHeaders() throws Exception {
        ...
        org.assertj.core.api.Assertions.assertThat(headers).hasSize(3).containsOnlyKeys("auth", "dochiaFuzzedHeader", "header");
    }
    ...
    void shouldThrowExceptionAsInvalidFormatForHeaders() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(filesArguments::loadHeaders).isInstanceOf(DochiaException.class)
