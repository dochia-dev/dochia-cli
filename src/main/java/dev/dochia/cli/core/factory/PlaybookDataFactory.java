 ...
 */
@ApplicationScoped
public class PlaybookDataFactory {
    private static final int RESPONSES_ARRAY_SIZE = 1;
    private static final int REQUEST_ARRAY_SIZE = 2;

    private final PrettyLogger logger = PrettyLoggerFactory.getLogger(PlaybookDataFactory.class);
     ...
     */
    @Inject
    public PlaybookDataFactory(FilesArguments filesArguments, ProcessingArguments processingArguments, GlobalContext globalContext, ValidDataFormat validDataFormat, FilterArguments filterArguments) {
     ...
     * @return a list of FuzzingData items representing a template that will be used to apply the Playbooks on
     */
    public List<PlaybookData> fromPathItem(String path, PathItem item, OpenAPI openAPI) {
        ...
        if (item.getPost() != null) {
            ...
            playbookDataList.addAll(this.getFuzzDataForPost(path, item, item.getPost(), openAPI));
        }

        if (item.getPut() != null) {
            logger.debug("Identified PUT method for path {}", path);
            playbookDataList.addAll(this.getFuzzDataForPut(path, item, item.getPut(), openAPI));
        }

        if (item.getPatch() != null) {
            logger.debug("Identified PATCH method for path {}", path);
            playbookDataList.addAll(this.getFuzzDataForPatch(path, item, item.getPatch(), openAPI));
        }

        if (item.getGet() != null) {
            logger.debug("Identified GET method for path {}", path);
            playbookDataList.addAll(this.getFuzzingDataForGet(path, item, item.getGet(), openAPI));
        }

        if (item.getDelete() != null) {
            logger.debug("Identified DELETE method for path {}", path);
            playbookDataList.addAll(this.getFuzzingDataForDelete(path, item, item.getDelete(), openAPI));
        }

        return playbookDataList;
    }
    ...
    private Schema<?> createSyntheticSchemaForGet(List<Parameter> operationParameters) {
        Schema<?> syntheticSchema = DochiaModelUtils.newObjectSchema();
    ...
    private List<PlaybookData> getFuzzDataForPost(String path, PathItem item, Operation operation, OpenAPI openAPI) {
        return this.getFuzzDataForHttpMethod(path, item, operation, HttpMethod.POST, openAPI);
    }

    private List<PlaybookData> getFuzzDataForPut(String path, PathItem item, Operation operation, OpenAPI openAPI) {
        return this.getFuzzDataForHttpMethod(path, item, operation, HttpMethod.PUT, openAPI);
    }


    private List<PlaybookData> getFuzzDataForPatch(String path, PathItem item, Operation operation, OpenAPI openAPI) {
        return this.getFuzzDataForHttpMethod(path, item, operation, HttpMethod.PATCH, openAPI);
    }

    private List<PlaybookData> getFuzzingDataForDelete(String path, PathItem item, Operation operation, OpenAPI openAPI) {
        return getFuzzDataForNonBodyMethods(path, item, operation, openAPI, HttpMethod.DELETE);
    }

    private List<PlaybookData> getFuzzingDataForGet(String path, PathItem item, Operation operation, OpenAPI openAPI) {
     ...
     */
    private List<PlaybookData> getFuzzDataForHttpMethod(String path, PathItem item, Operation operation, HttpMethod method, OpenAPI openAPI) {
        ...
        if (mediaType == null) {
            logger.warn("Content type not supported for path {}, method {}. dochia detects application/json by default. " + "You might try to supply the custom content type using --contentType argument", path, method);
            return Collections.emptyList();
        }
        ...
        for (String reqSchemaName : reqSchemaNames) {
            ...
            playbookDataList.addAll(generationResult.examplePayloads().stream()
     ...
     */
    private List<PlaybookData> getFuzzDataForNonBodyMethods(String path, PathItem item, Operation operation, OpenAPI openAPI, HttpMethod method) {
        globalContext.recordPathAndMethod(path, method);
        if (this.isDeprecated(operation)) {
    ...
    Set<Object> extractExamples(MediaType mediaType) {
        ...
        if (mediaType.getExamples() != null) {
            ...
            examples.addAll(mediaType.getExamples().values()
                    ...
                    .map(example -> {
                        Example exampleFromSchemaMap = globalContext.getExampleMap().get(DochiaModelUtils.getSimpleRef(example.get$ref()));
    ...
    private List<String> getCurrentRequestSchemaName(MediaType mediaType) {
        ...
        if (mediaType.getSchema() != null) {
            ...
            if (currentRequestSchema == null && DochiaModelUtils.isArraySchema(mediaType.getSchema())) {
                currentRequestSchema = mediaType.getSchema().getItems().get$ref();
            }
            if (currentRequestSchema != null) {
                reqSchemas.add(DochiaModelUtils.getSimpleRef(currentRequestSchema));
            } else if (DochiaModelUtils.isComposedSchema(mediaType.getSchema())) {
    ...
    private String calculateSchemaRef(Schema innerSchema) {
        ...
        return DochiaModelUtils.getSimpleRef(innerSchema.get$ref());
    }
    ...
    private MediaType getMediaType(Operation operation, OpenAPI openAPI) {
        for (String contentType : processingArguments.getContentType()) {
            if (operation.getRequestBody() != null && operation.getRequestBody().get$ref() != null) {
                ...
                RequestBody requestBody = openAPI.getComponents().getRequestBodies().get(DochiaModelUtils.getSimpleRef(reqBodyRef));
    ...
    private GenerationResult getRequestPayloadsSamples(MediaType mediaType, String reqSchemaName) {
        ...
        if (mediaType != null && DochiaModelUtils.isArraySchema(mediaType.getSchema())) {
    ...
    private List<String> generateSample(String reqSchemaName, OpenAPIModelGenerator generator) {
        String onlySchemaName = DochiaModelUtils.getSimpleRef(reqSchemaName);
    ...
    private List<String> getRequestContentTypes(Operation operation, OpenAPI openAPI) {
        ...
        if (operation.getRequestBody() != null) {
            ...
            String reqBodyRef = operation.getRequestBody().get$ref();
            if (reqBodyRef != null) {
                operation.getRequestBody().setContent(openAPI.getComponents().getRequestBodies().get(DochiaModelUtils.getSimpleRef(reqBodyRef)).getContent());
            }
