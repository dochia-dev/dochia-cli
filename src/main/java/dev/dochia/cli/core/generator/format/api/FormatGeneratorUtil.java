...
public final class FormatGeneratorUtil {
    ...
    public static char randomLetter() {
        return (char) ('A' + DochiaRandom.instance().nextInt(26));
    }
    ...
    public static int randomDigit() {
        return DochiaRandom.instance().nextInt(10);
    }
    ...
    public static int randomInRange(int min, int max) {
        return DochiaRandom.instance().nextInt(max - min) + min;
    }
    ...
    public static String randomAlphanumeric(int length) {
        StringBuilder sb = new StringBuilder(length);
        for (int i = 0; i < length; i++) {
            if (DochiaRandom.instance().nextBoolean()) {
                sb.append(randomDigit());
            } else {
                sb.append(randomLetter());
            }
    ...
    public static String generateFromPattern(String pattern) {
        ...
        for (int i = 0; i < pattern.length(); i++) {
            ...
            switch (c) {
                ...
                case 'X' -> result.append(DochiaRandom.instance().nextBoolean() ? randomDigit() : randomLetter());
