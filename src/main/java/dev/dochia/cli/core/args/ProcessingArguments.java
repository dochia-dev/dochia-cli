...
/**
 * Holds arguments related to different customizations supported when running the Playbooks.
 */
@Singleton
@Getter
public class ProcessingArguments {
    @CommandLine.Option(names = {"--fields-selection-strategy"}, paramLabel = "<strategy>",
            description = "The strategy used by some of the field playbooks. Default: @|bold,underline ${DEFAULT-VALUE}|@")
    private SetSelectionStrategy fieldsSelectionStrategy = SetSelectionStrategy.ONEBYONE;

    @CommandLine.Option(names = {"--max-fields-to-remove"}, paramLabel = "<max>",
    ...
    @CommandLine.Option(names = {"--edge-spaces-strategy"}, paramLabel = "<strategy>",
            description = "This can be either @|bold,underline VALIDATE_AND_TRIM|@ or @|bold,underline TRIM_AND_VALIDATE|@. It can be used to specify what it should expected when sending trailing and leading spaces valid values within fields. Default: @|bold,underline ${DEFAULT-VALUE}|@")
    private TrimmingStrategy edgeSpacesStrategy = TrimmingStrategy.TRIM_AND_VALIDATE;

    @CommandLine.Option(names = {"--sanitization-strategy"}, paramLabel = "<strategy>",
            description = "This can be either @|bold,underline SANITIZE_AND_VALIDATE|@ or @|bold,underline VALIDATE_AND_SANITIZE|@. It can be used to specify what it should expected when sending Unicode Control Chars and Other Symbols within the fields. Default: @|bold,underline ${DEFAULT-VALUE}|@")
    ...
    @CommandLine.Option(names = {"--use-request-body-examples"},
    ...
    @CommandLine.Option(names = {"--use-response-body-examples"}, negatable = true, defaultValue = "true", fallbackValue = "true",
    ...
    @CommandLine.Option(names = {"--use-schema-examples"},
    ...
    @CommandLine.Option(names = {"--use-property-examples"}, negatable = true, defaultValue = "true", fallbackValue = "true",
    ...
    @CommandLine.Option(names = {"--cache-payloads"}, negatable = true, defaultValue = "true", fallbackValue = "true",
    ...
    @CommandLine.Option(names = {"--large-strings-size"}, paramLabel = "<size>",
    ...
    @Setter
    @CommandLine.Option(names = {"--self-reference-depth", "-L"}, paramLabel = "<depth>",
    ...
    @Setter
    @CommandLine.Option(names = {"--content-type"}, paramLabel = "<content-type>",
    ...
    @Setter
    @CommandLine.Option(names = {"--allow-invalid-enum-values"},
            description = "When set to @|bold true|@, the InvalidValuesInEnumsFieldsPlaybook will expect a 2XX response code instead of 4XX. Default: @|bold,underline ${DEFAULT-VALUE}|@")
    ...
    @CommandLine.Option(names = {"--http2-prior-knowledge"},
