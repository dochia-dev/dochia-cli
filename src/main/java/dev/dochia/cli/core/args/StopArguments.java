...
public class StopArguments {
    @CommandLine.Option(
            names = {"--stop-after-time-in-sec", "--st"}, paramLabel = "<max-time>",
    ...
    @CommandLine.Option(
            names = {"--stop-after-errors", "--se"}, paramLabel = "<max-errors>",
    ...
    @CommandLine.Option(
            names = {"--stop-after-mutations", "--sm"}, paramLabel = "<max-mutations>",
    ...
    public boolean shouldStop(long errors, long tests, long startTimeInMs) {
        return isErrorThresholdTriggered(errors)
                || isNumberOfTestsThresholdTriggered(tests)
