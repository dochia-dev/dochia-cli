 ...
 * Allows flexible control over when DOCHIA should exit with error codes based on test results.
 */
@Singleton
@Getter
public class QualityGateArguments {
    ...
    @CommandLine.Option(names = {"--fail-on"},
            description = "Comma-separated list of conditions that should cause dochia to exit with code 1. " +
                    "Valid values: @|bold error|@, @|bold warn|@. Default: @|bold error|@. " +
                    "Example: @|bold --fail-on error,warn|@ will exit 1 on any error or warning")
    private String failOn;

    @CommandLine.Option(names = {"--quality-gate"},
            description = "Comma-separated list of threshold conditions. Format: @|bold metric<threshold|@ or @|bold metric>threshold|@. " +
                    ...
                    "Example: @|bold --quality-gate \"errors<5,warns<20\"|@ will exit 1 if errors >= 5 or warns >= 20")
    private String qualityGate;

    /**
     * Evaluates whether DOCHIA should exit with an error code based on the configured quality gates.
     *
