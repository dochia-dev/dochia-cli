...
public class GlobalContext {
    ...
    private final PrettyLogger logger = PrettyLoggerFactory.getLogger(GlobalContext.class);
    ...
    private final Properties playbooksConfiguration = new Properties();
    ...
    private DochiaConfiguration dochiaConfiguration;
    @Setter
     ...
     */
    public String getExpectedResponseCodeConfigured(String playbook) {
        return this.playbooksConfiguration.getProperty(playbook);
    }

    /**
     * Initializes the global context with the OpenAPI document, content types, playbooks configuration and dochia configuration.
     *
     * @param openAPI                the OpenAPI document
     * @param contentType            the content types
     * @param playbooksConfiguration the playbooks configuration
     * @param dochiaConfiguration    the dochia configuration
     */
    public void init(OpenAPI openAPI, List<String> contentType, Properties playbooksConfiguration, DochiaConfiguration dochiaConfiguration,
                     Set<String> errorLeaksKeywords) {
        ...
        this.getPlaybooksConfiguration().putAll(playbooksConfiguration);
    ...
    public void writeRecordedErrorsIfPresent() {
        ...
        consoleLogger.noFormat(ansi().bold().fgRed().a("\nThere were errors during playbooks execution. It's recommended to fix them in order for all playbooks to properly run: ").reset().toString());
    ...
    private Object extractFromSchema(String part, Schema<?> schema, Object resolvedDefinition) {
        ...
        if (ofs.contains(part.toLowerCase(Locale.ROOT))) {
            if (!DochiaModelUtils.isComposedSchema(schema)) {
    ...
    private Schema getSchemaFromComponentsDefinitions(String reference) {
        return schemaMap.get(DochiaModelUtils.getSimpleRef(reference));
    }
