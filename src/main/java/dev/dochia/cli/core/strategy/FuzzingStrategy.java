...
public abstract sealed class FuzzingStrategy permits InsertFuzzingStrategy, NoopFuzzingStrategy,
     ...
     */
    public static List<FuzzingStrategy> getFuzzingStrategies(PlaybookData data, String fuzzedField, List<String> invisibleChars, boolean maintainSize) {
        ...
        final boolean insertWithoutReplace = !maintainSize || !CommonUtils.isEmpty(fuzzedFieldSchema.getEnum());

        return invisibleChars.stream()
                .map(value -> FuzzingStrategy.replace()
                        .withData(CommonUtils.insertInTheMiddle(initialValue, value, insertWithoutReplace)))
                .toList();
    }
    ...
    public static FuzzingResult replaceField(String payload, String jsonPropertyForReplacement, FuzzingStrategy fuzzingStrategyToApply, boolean mergeFuzzing) {
        if (StringUtils.isNotBlank(payload)) {
            ...
            if (JsonUtils.isJsonArray(payload)) {
                jsonPropToGetValue = CommonUtils.isRootArray(jsonPropertyForReplacement) ?
                        jsonPropertyForReplacement : JsonUtils.FIRST_ELEMENT_FROM_ROOT_ARRAY + jsonPropertyForReplacement;
                jsonPropertyForReplacement = CommonUtils.isRootArray(jsonPropertyForReplacement) ?
            ...
            CommonUtils.replaceOldValueWithNewOne(jsonPropertyForReplacement, jsonDocument, valueToSet);
