...
/**
 * Minimal replacement for the Spring AnnotationUtils calls used by Dochia.
 *
 * <p>Supports: - isAnnotationDeclaredLocally(): only direct presence - findAnnotation(): direct +
 * meta-annotations; optionally scans superclasses and interfaces
 *
 * <p>Notes: - This is intentionally small and safe for native-image. - It avoids deep Spring
 * semantics like @AliasFor.
 */
public abstract class AnnotationUtils {
  ...
  public static boolean isAnnotationDeclaredLocally(
      Class<? extends Annotation> annotationType, Class<?> targetType) {
   ...
   *
   * <p>Scans superclasses and interfaces if @Inherited applies OR if scanHierarchy is true.
   *
   * @param targetType the target class to scan
  ...
  public static <A extends Annotation> A findAnnotation(
      Class<?> targetType, Class<A> annotationType) {
    return findAnnotation(targetType, annotationType, true);
  }

  public static <A extends Annotation> A findAnnotation(
      Class<?> targetType, Class<A> annotationType, boolean scanHierarchy) {
    Objects.requireNonNull(targetType, "targetType");
    Objects.requireNonNull(annotationType, "annotationType");
  ...
  private static <A extends Annotation> A findInMetaAnnotations(
      Class<?> current, Class<A> annotationType) {
