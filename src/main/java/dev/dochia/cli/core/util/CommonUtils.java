...
public abstract class CommonUtils {
  ...
  /**
   * Filters a collection based on a specified predicate and prints items that do not match the
   * predicate using a logger. Returns a list of items that match the predicate.
   *
   ...
   * @param functionToApplyToLoggedItems a function to apply to logged items for additional
   *     information
   * @param params additional parameters to include in the log message
   * @return a list of items that match the predicate
   */
  public static <T> List<T> filterAndPrintNotMatching(
      Collection<T> collection,
      Predicate<T> predicateToFilter,
      PrettyLogger logger,
      String messageWhenNotMatching,
      Function<T, String> functionToApplyToLoggedItems,
      String... params) {
    Map<Boolean, List<T>> results =
        collection.stream().collect(Collectors.partitioningBy(predicateToFilter));
    ...
    notMatching.forEach(
        element ->
            logger.skip(
                messageWhenNotMatching, functionToApplyToLoggedItems.apply(element), params));

    return results.get(true);
  }

  /**
   * Sets the log level for dochia packages.
   *
   * @param level the desired log level (e.g., "DEBUG", "INFO", "WARN", "ERROR")
   */
  public static void setDochiaLogLevel(String level) {
    setLogLevel("dev.dochia.cli", level);
  }
   ...
   */
  public static FuzzingResult justReplaceField(
      String payload, String jsonPropertyForReplacement, Object with) {
    if (JsonUtils.isJsonArray(payload)) {
      jsonPropertyForReplacement =
          isRootArray(jsonPropertyForReplacement)
              ? jsonPropertyForReplacement
              : JsonUtils.ALL_ELEMENTS_ROOT_ARRAY + jsonPropertyForReplacement;
    }
  ...
  public static void replaceOldValueWithNewOne(
      String jsonPropertyForReplacement, DocumentContext jsonDocument, Object valueToSet) {
    if (JsonUtils.isValidJson(elementToString(valueToSet))) {
      ...
      try {
        jsonDocument.set(
            JsonUtils.sanitizeToJsonPath(jsonPropertyForReplacement),
            JsonUtils.JSON_PERMISSIVE_PARSER.parse(String.valueOf(valueToSet)));
  ...
  private static boolean areBothPropertyToReplaceAndValueToReplaceArrays(
      String jsonPropertyForReplacement, Object valueToSet) {
   ...
   */
  public static String setAdditionalPropertiesToPayload(
      Map<String, Object> currentPathValues, String payload) {
    Set<String> additionalPropertiesKeys =
        currentPathValues.keySet().stream()
            .filter(key -> key.matches(ADDITIONAL_PROPERTIES))
            .collect(Collectors.toSet());
    String result = payload;

    for (String additionalPropertiesKey : additionalPropertiesKeys) {
      String additionalProperties =
          WordUtils.nullOrValueOf(currentPathValues.get(additionalPropertiesKey));
  ...
  private static void setMapValues(
      DocumentContext jsonDoc, String additionalProperties, String prefix) {
    String mapValues =
        additionalProperties.replace(MAP_VALUES + "=", "").replace("{", "").replace("}", "");
    for (String values : mapValues.split(",", -1)) {
      String[] entry = values.split("=", -1);
      jsonDoc.put(
          JsonPath.compile(prefix),
          entry[0].trim(),
          DSLParser.parseAndGetResult(
              entry[1].trim(), Map.of(Parser.REQUEST, jsonDoc.jsonString())));
    }
  }
   ...
   *
   * <p>This method checks if the initial value can be converted to a numeric type (such as Integer,
   * Long, Double, etc.) using the {@link
   * org.apache.commons.lang3.math.NumberUtils#isCreatable(String)} method. If the conversion is
   * possible, it returns the converted numeric value; otherwise, it returns the initial value as
   * is.
   *
   ...
   * @param insertWithoutReplace If true, the insertion is done without replacing any characters; if
   *     false, the insertion replaces a portion of the original string.
   * @return The string resulting from the insertion operation.
   * @throws NullPointerException If 'value' or 'whatToInsert' is null.
   */
  public static String insertInTheMiddle(
      String value, String whatToInsert, boolean insertWithoutReplace) {
    ...
    return value.substring(0, position - diffBasedOnReplace)
        + whatToInsert
        + value.substring(position + diffBasedOnReplace);
  }

  /**
   * Returns the maximum number of array elements that can be generated based on the length of the
   * field value.
   *
   * @param fieldValue The field value to be used for calculating the maximum number of array
   *     elements.
   ...
   */
  public static Comparator<Map.Entry<String, PathItem>> createCustomComparatorBasedOnPathsOrder(
      List<String> pathsOrder) {
    return (e1, e2) -> {
  ...
  /**
   * Returns a new string with randomized casing, different from the original. Non-alphabetic
    ...
    do {
      ...
      for (int i = 0; i < input.length(); i++) {
        ...
        if (Character.isLetter(ch)) {
          result.append(
              random().nextBoolean() ? Character.toLowerCase(ch) : Character.toUpperCase(ch));
        } else {
          result.append(ch);
        }
      }
