...
public interface ResponseCodeFamily {
  ...
  static boolean isValidCode(String code) {
    ...
    return Character.isDigit(firstDigit)
        && Character.digit(firstDigit, 10) >= 1
        && Character.digit(firstDigit, 10) <= 5;
  }
  ...
  /**
   * Checks if two response codes match as an exact match, a generic range match, or a reversed
   * generic range match.
   *
   * @param codeOne The first response code.
   * @param codeTwo The second response code.
   * @return {@code true} if the codes match as an exact match, a generic range match, or a reversed
   *     generic range match, {@code false} otherwise.
   */
  static boolean matchAsCodeOrRange(String codeOne, String codeTwo) {
    return codeOne.equalsIgnoreCase(codeTwo)
        || (codeOne.substring(1, 3).equalsIgnoreCase(XX)
            && codeOne.substring(0, 1).equalsIgnoreCase(codeTwo.substring(0, 1)))
        || (codeTwo.substring(1, 3).equalsIgnoreCase(XX)
            && codeTwo.substring(0, 1).equalsIgnoreCase(codeOne.substring(0, 1)));
  }
   ...
   *
   * @param code The response code to check.
   * @return {@code true} if the code matches any allowed response codes or their generic forms,
  ...
  default boolean matchesAllowedResponseCodes(String code) {
    ...
    boolean isAllowedGenericResponseCode =
        this.allowedResponseCodes().stream()
            .filter(allowedCode -> allowedCode.endsWith(XX))
            .anyMatch(
                allowedCode -> allowedCode.charAt(0) == code.toUpperCase(Locale.ROOT).charAt(0));
