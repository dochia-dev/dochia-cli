 ...
 */
public abstract class ExactValuesInFieldsPlaybook extends BaseBoundaryFieldPlaybook {
     ...
     */
    protected ExactValuesInFieldsPlaybook(ServiceCaller sc, TestCaseListener lr, FilesArguments cp) {
        super(sc, lr, cp);
    }
     ...
     * There are 2 cases when this method gets involved (see {@link BaseBoundaryFieldPlaybook#isFieldFuzzable(String, PlaybookData)}):
     * <ol>
     ...
     * as those scenarios are already covered by the HappyPathPlaybook.
     *
    ...
    private String generateWithAdjustedLength(Schema schema, int adjustedLength) {
        ...
        if (DochiaModelUtils.isByteArraySchema(schema)) {
