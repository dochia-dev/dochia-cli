...
public class RandomAbugidasMutator implements BodyMutator {
    ...
    public String mutate(String inputJson, String selectedField) {
        ...
        return CommonUtils.justReplaceField(inputJson, selectedField, valueWithAbugidas).json();
    }
    ...
    public static String generateRandomTeluguCharacter() {
        String consonant = TELUGU_CONSONANTS[CommonUtils.random().nextInt(TELUGU_CONSONANTS.length)];
        String vowel = TELUGU_VOWELS[CommonUtils.random().nextInt(TELUGU_VOWELS.length)];
        String modifier = TELUGU_MODIFIERS[CommonUtils.random().nextInt(TELUGU_MODIFIERS.length)];

        return consonant + "\u200C" + vowel + modifier;
    }

    public static String generateRandomBengaliCharacter() {
        String consonant = BENGALI_CONSONANTS[CommonUtils.random().nextInt(BENGALI_CONSONANTS.length)];
        String vowel = BENGALI_VOWELS[CommonUtils.random().nextInt(BENGALI_VOWELS.length)];
        String modifier = BENGALI_MODIFIERS[CommonUtils.random().nextInt(BENGALI_MODIFIERS.length)];
