 ...
 * {"id":1,"id":"dochiaFuzzyDup"}.
 * Detects first-wins / last-wins ambiguities per RFC 8259.
 */
@FieldPlaybook
@Singleton
public class DuplicateKeysFieldsPlaybook implements TestCasePlaybook {

    private static final PrettyLogger logger = PrettyLoggerFactory.getLogger(DuplicateKeysFieldsPlaybook.class);
    ...
    public void run(PlaybookData data) {
        if (JsonUtils.isEmptyPayload(data.getPayload())) {
            logger.debug("Skipping playbook for path {} - empty payload", data.getPath());
            return;
        }
    ...
    private void executeFieldDuplication(PlaybookData data, String field) {
