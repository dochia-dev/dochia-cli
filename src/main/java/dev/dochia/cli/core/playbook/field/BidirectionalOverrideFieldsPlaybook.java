...
@FieldPlaybook
@Singleton
public class BidirectionalOverrideFieldsPlaybook extends BaseReplaceFieldsPlaybook {
     ...
     */
    public BidirectionalOverrideFieldsPlaybook(FieldsIteratorExecutor ce, ProcessingArguments pa) {
        super(ce);
    ...
    public BaseReplaceFieldsContext getContext(PlaybookData data) {
        ...
        return BaseReplaceFieldsContext.builder()
                ...
                .skipMessage("Playbook only runs for String non-enum fields")
                .fieldFilter(field -> {
                    Schema<?> schema = data.getRequestPropertyTypes().get(field);
                    return DochiaModelUtils.isStringSchema(schema) && !DochiaModelUtils.isEnumSchema(schema) && JsonUtils.isFieldInJson(data.getPayload(), field);
                })
