package dev.dochia.cli.core.openapi;
...
public class OpenAPIModelGenerator {
    ...
    private <T> Object getExampleFromStringSchema(String propertyName, Schema<T> property) {
        ...
        if (DochiaModelUtils.isDecimalSchema(property)) {
    ...
    private boolean isAnyComposedSchemaInChain(String schemaChain) {
        ...
        return Arrays.stream(schemaRefs).anyMatch(entry -> DochiaModelUtils.isComposedSchema(globalContext.getSchemaFromReference(entry)));
    }
    ...
    private Schema normalizeDiscriminatorMappingsToOneOf(String name, Schema<?> schema) {
        ...
        if (schema != null && schema.getDiscriminator() != null && !CollectionUtils.isEmpty(schema.getDiscriminator().getMapping())
            ...
            newSchema.setName("DochiaChanged" + name);
            ...
            for (String oneOfSchemaRef : schema.getDiscriminator().getMapping().values()) {
                String oneOfSchema = DochiaModelUtils.getSimpleRef(oneOfSchemaRef);
    ...
    private List<Map<String, Object>> generateExamplesForSchema(String name, Schema schema) {
        ...
        if (DochiaModelUtils.isAllOf(schema) || DochiaModelUtils.isAllOfWithProperties(schema)) {
    ...
    private List<Map<String, Object>> resolveAllOfSchemaProperties(Schema schema, String propertyName) {
        ...
        for (Schema subSchema : schemas) {
            ...
            if (DochiaModelUtils.isAnyOf(subSchema) || DochiaModelUtils.isOneOf(subSchema)) {
    ...
    private List<Map<String, Object>> resolveArraySchemaProperties(String propertyName, Schema schema) {
        ...
        List<Map<String, Object>> examples = new ArrayList<>();
        Schema itemSchema = DochiaModelUtils.getSchemaItems(schema);
    ...
    private List<Object> resolvePropertyToExamples(String propertyName, Schema property) {
        ...
        if (DochiaModelUtils.isArraySchema(property)) {
            createExamplesArray(propertyName, property, examples);
        } else if (DochiaModelUtils.isMapSchema(property)) {
            Map<String, Object> mapExample = new HashMap<>();
            Schema additionalProperties = DochiaModelUtils.getAdditionalProperties(property);
            ...
            if (resolved != null && DochiaModelUtils.isPrimitiveSchema(resolved)) {
        ...
        } else if (DochiaModelUtils.isObjectSchemaUsingOAT(property)) {
            examples.addAll(generateExamplesForSchema(propertyName, property));
        } else if (DochiaModelUtils.isAllOf(property) || DochiaModelUtils.isAllOfWithProperties(property)) {
            examples.addAll(resolveAllOfSchemaProperties(property, propertyName));
        } else if (DochiaModelUtils.isOneOf(property) || DochiaModelUtils.isAnyOf(property)) {
    ...
    private void createExamplesArray(String propertyName, Schema property, List<Object> examples) {
        if (Boolean.TRUE.equals(property.getUniqueItems())) {
            ...
            Schema itemSchema = DochiaModelUtils.getSchemaItems(property);
    ...
    private void keepOriginalSchema(String propertyName, Schema<?> composedSchema) {
        String keyToKeep = DochiaModelUtils.getSimpleRef(propertyName) + GlobalContext.ORIGINAL;
    ...
    private void mergeRequiredProperties(List<Schema> allOfSchema) {
        ...
        Collection<Schema> allOfSchemasSanitized = allOfSchema
                .stream()
                .filter(DochiaModelUtils::isNotEmptySchema)
    ...
    private <T> Object generateStringValue(String propertyName, Schema<T> propertySchema) {
        if (propertyName == null || syntheticSchema(propertyName) || !DochiaModelUtils.isStringSchema(propertySchema)) {
            return null;
        }
    ...
    <T> Object formatExampleIfNeeded(Schema<T> property, Object example) {
        ...
        if (DochiaModelUtils.isDateSchema(property)) {
        ...
        if (DochiaModelUtils.isDateTimeSchema(property)) {
    ...
    private <T> Object generateExampleBySchemaType(String propertyName, Schema<T> propertySchema) {
        if (DochiaModelUtils.isStringSchema(propertySchema)) {
            return this.getExampleFromStringSchema(propertyName, propertySchema);
        } else if (DochiaModelUtils.isBooleanSchema(propertySchema)) {
            return this.getExampleFromBooleanSchema();
        } else if (DochiaModelUtils.isNumberSchema(propertySchema)) {
            return this.getExampleFromNumberSchema(propertySchema);
        } else if (DochiaModelUtils.isIntegerSchema(propertySchema)) {
            return this.getExampleFromIntegerSchema(propertySchema);
        } else if (DochiaModelUtils.isObjectSchema(propertySchema)) {
    ...
    private Object getExampleFromIntegerSchema(Schema<?> property) {
        ...
        if (DochiaModelUtils.isShortIntegerSchema(property)) {
    ...
    private Object getExampleFromNumberSchema(Schema<?> property) {
        ...
        if (DochiaModelUtils.isFloatSchema(property)) {
    ...
    private void recordRequestSchema(String propertyName, Schema<?> schema) {
        ...
        schema.addExtension(DochiaModelUtils.X_DOCHIA_FIELD_NAME, propertyName);
